...
const PHASES: {
...
}[] = [
  {
    ...
    frame: '대구시 협조 없이 자체 실증 가능',
    status: 'live',
    owner: '오큐브 자체 자산 (공단 SDK · eTAS 등록)',
    items: [
      { source: 'DTG', desc: '위험운전 탐지 · 기사별 운전습관 분석 · 사고위험 점수화' },
      { source: 'OBD/CAN', desc: '엔진·배터리·브레이크 이상탐지, 고장예측' },
  ...
  },
  {
    ...
    status: 'next',
    owner: 'AFC = 유페이 정산 채널 · 민원 = 대구시',
    items: [
      { source: 'AFC/교통카드', desc: '정류장별 수요, OD, 환승, 요금·수입 분석' },
      { source: 'APC/승객계수', desc: '실시간 혼잡도, 승하차 인원, 좌석·입석 추정' },
  ...
  },
  {
    ...
    items: [
      { source: 'DVR/CCTV', desc: '사고·승객 이상행동·문 끼임·차선이탈·보행자 위험 탐지' },
      { source: 'BIS', desc: '도착예측, 정류장별 지연 예측, 시민 안내정보 고도화' },
      { source: 'BMS', desc: '배차 최적화, 회차·우회·예비차 투입 추천' },
      { source: 'ITS', desc: '도로 정체, 신호, 사고, 공사 반영 운행 예측' },
    ],
  },
]
    ...
    <div className="mx-auto flex h-full max-w-6xl flex-col gap-4 overflow-y-auto pr-1">
      <div className="px-1 pt-1">
        ...
        <p className="mt-1 text-sm text-gray-500">
          ...
          컨소 자산(2차) → 도시 협조(3차). 단계가 쌓일수록 후발주자가 모방 불가능한 통합 데이터 해자가 된다.
        </p>
      </div>
